...
const Hero = ({ settings }) => {
  ...
  const badge = (size) => (
    <span
      className={
        size === 'sm'
          ? 'mb-2 inline-block rounded-full border border-gold/40 bg-gold/10 px-3 py-0.5 text-xs font-medium text-gold'
          : 'mb-4 inline-block rounded-full border border-gold/40 bg-gold/10 px-4 py-1 text-sm font-medium text-gold'
      }
    >
  ...
  return (
    <section className="bg-black">
      {/* Mobile: image and text never overlap - image is a fixed-height block up
          top (hood always fully visible), text sits in normal flow below it. */}
      <div className="sm:hidden">
        ...
        <div className="bg-black px-5 pb-8 pt-6">
          {badge('sm')}
